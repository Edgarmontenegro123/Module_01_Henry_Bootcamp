...
En un primer momento, se creará el contexto global, donde van a estar definida la función `creaFuncion` y también el arreglo `arr`.
...
Veamos el siguiente código, primero definimos una función que va a retornar otra función (esta sería nuestra _fábrica de funciones_), esta recibe como parámetro el lenguaje del saludo, y retorna una función que salude (console.log) en el idioma recibido.
...
Ahora que sabemos lo que son los _closures_, si pensamos en todo lo que hicimos alguna vez con JavaScript, es muy probable que nos demos cuenta de que ya lo venimos usando (tal vez sin saberlo).
...
En el ejemplo de arriba, cuando invocamos a `saludarMasTarde` estamos creando un execution context, en el que invocamos a la función `setTimeout` y donde definimos la variable `saludo`. Ese execution context es destruido, pero `setTimeout` contiene una referencia a `saludo`. ¿Closure, Maybe?
...
¡O sea que se hicieron algo parecido a esto (tal vez usando eventos), entonces ya usaron _functions expressions_ y muy probablemente _closures_ también!

## Call, Apply and Bind.

Cuando vimos el keyword `this`, dijimos que el intérprete era el que manejaba el valor de este. Bueno, esto no es del todo cierto, hay una serie de variables que nos van a permitir poder setear nosotros el keyword `this`.

Como en JavaScript las funciones son un tipo objeto especial (vimos que tenían algunas propiedades especiales como `code` y `name`), estas también contienen métodos propios. Todas las funciones tienen acceso a los métodos:
...
Si usamos `bind()` entonces la nueva función queda __siempre__ ligada al objeto que pasamos cómo argumento. O sea que si quisiéramos usarla para otro objeto, tendríamos que crear una nueva _copia_ de la función y _bindearle_ un nuevo objeto.
...
El primer argumento de `call` es el objeto a usar cómo `this`. Después de este puedo pasar otros argumentos, que serán pasados a la función que estamos invocando. Por ejemplo, si nuestra función recibiera argumentos, usaríamos `call` de la siguiente manera:
...
Como `bind` crea una nueva función, si le pasamos parámetros, estos quedan __fijos__ en la nueva función. En el ejemplo no vamos a bindear `this` con nada, pero sí unos parámetros.
