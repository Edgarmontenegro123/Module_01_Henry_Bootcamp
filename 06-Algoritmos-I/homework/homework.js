...
function bubbleSort(array) {
  ...
  //     if(array[j] > array[i]){ //Comparo los valores adyacentes y en caso de que el primero sea mayor:
  ...
  //         array[i] = aux;      // Regreso mi valor guardado temporalmente en la variable aux
  //     }
  ...
  while(swap){ // Pequeña optimización ya que si el array está organizado while se da cuenta y for continua el ciclo
    ...
    for(let i = 0; i < array.length - 1; i++){
      if(array[i] > array[i + 1]){
        ...
        array[i] = array[i + 1];      // Sobreescribo mi array[i] por el valor siguiente
        array[i + 1] = aux;           // Sobreescribo el valor de array[ i + 1] con el valor guardado en la variable aux
...
function insertionSort(array){
  ...
  //-----------------------------------------------------------------------------------------------------------------------
    for(let i = 1; i < array.length; i++){ // Recorro el array con el puntero i
      ...
      while(j >= 0 && array[j] > aux){     // Mientras la posición index de j sea >= 0, y el valor sea mayor que el aux entro al ciclo
